...
PROGRAM=${0##*/}
case "${EMAILWIZ_RUNTIME_MODE:-production}" in
	development)
	...
	production)
		...
		STATE_DIR=/var/lib/emailwiz
		DB_PATH=$STATE_DIR/emailwiz.sqlite3
		DNS_DIR=$STATE_DIR/dns
		DOVECOT_CONF=/etc/dovecot/dovecot.conf
		DOVECOT_SQL_CONF=/etc/dovecot/emailwiz-sql.conf.ext
		POSTFIX_DIR=/etc/postfix
		OPENDKIM_KEYTABLE=$POSTFIX_DIR/dkim/keytable
		OPENDKIM_SIGNINGTABLE=$POSTFIX_DIR/dkim/signingtable
		DKIM_ROOT=$POSTFIX_DIR/dkim
		SIEVE_DIR=/var/lib/dovecot/sieve
		HOME_ROOT=/home
		RENEW_HOOK=/etc/letsencrypt/renewal-hooks/deploy/emailwiz
		OS_RELEASE_FILE=/etc/os-release
		SQLITE3=sqlite3
		;;
	*)
		printf 'ERROR: Invalid Emailwiz runtime mode: %s\n' "$EMAILWIZ_RUNTIME_MODE" >&2
		exit 1
		;;
esac
